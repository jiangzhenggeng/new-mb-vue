...
<template>
  <div class="page">
    <page-header-back @back="back('/mb/user/login')" title="忘记密码"/>
        ...
        <div class="login__form-ctr">
          ...
          <div class="form__row">
            <label class="form__cell">
              ...
              <input type="text" name="vcode" v-model="vcode" ref="login-vcode" placeholder="图片校验码" autocomplete="off"/>
            ...
            <div class="validatecode w250 mgl10">
              <img
                ...
                alt="看不清？点击更换另一个验证码"
                :src="USER_VAR_CONFIG.passwdCode"
              />
            </div>
          </div>
          <div class="form__row tel-validatecode">
            <label class="form__cell">
              ...
              <input type="text" name="code" v-model="code" ref="login-code" placeholder="手机验证码" autocomplete="off"/>
              <i @click="clearInputContent('code')" v-if="code" class="icon-v2 icon-v2-clear-input right">
                <img src="./static/icon/clear-close.svg"/>
              </i>
            </label>
            <div class="w250 mgl10">
              <send-code :postData="{'action':'passwd',tel,vcode}" url="/api/validation/tel"/>
            </div>
          </div>
